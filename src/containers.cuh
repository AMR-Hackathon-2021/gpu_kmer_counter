...
template <typename T> class device_value {
public:
  // Default constructor
  device_value() {
  ...
  device_value(const device_value &other) {
    CUDA_CALL(cudaMalloc((void **)&data_, sizeof(T)));
    CUDA_CALL(cudaMemcpy(data_, other.data_, sizeof(T), cudaMemcpyDefault));
  }
  ...
  ~device_value() { CUDA_CALL_NOTHROW(cudaFree(data_)); }

  T get_value() const {
    T host_value;
    CUDA_CALL(cudaMemcpy(&host_value, data_, sizeof(T), cudaMemcpyDefault));
    return host_value;
  }
...
template <typename T> class interleaved {
public:
  __device__ interleaved(T *data, size_t offset, size_t stride)
      : data_(data + offset), stride_(stride) {}

  template <typename Container>
  __device__ interleaved(Container &data, size_t offset, size_t stride)
      : interleaved(data.data(), offset, stride) {}

  __device__ T &operator[](size_t i) { return data_[i * stride_]; }

  __device__ const T &operator[](size_t i) const { return data_[i * stride_]; }
